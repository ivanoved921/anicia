...
foreach ($art as $elem) {
    if($elem['author'] == $author && $elem['property'] == 1) {
        $artNew[]= $elem;
    };
}
            ...
            <div class="description col-md-5 col-xs-4">
                <div class="item_header">
                    <h3 class="item-description-title">
                        <a class="item-daescription-title-link" href="itemPage.php?id=<?= $task['id'];?>"><?php echo $task['num_of_rooms'].'-к квартира, '.$task['square'].' кв.м, '.$task['floor'].'/'.$task['floor_total'].' эт.';?></a>
                    </h3>
