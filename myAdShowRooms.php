...
session_start();
$db = new QueryBuilder;
$art = $db->allByProperty("apartments", 2);

//Выбираем записи по логину
$author = $_SESSION['login'];
foreach ($art as $elem) {
    if($elem['author'] == $author && $elem['property'] == 2) {
        $artNew[]= $elem;
    };
}
            ...
            <div class="description col-md-5 col-xs-4">
                <div class="item_header">
                    <h3 class="item-description-title">
                        <a class="item-daescription-title-link" href="itemPage.php?id=<?= $task['id'];?>"><?php echo "Комната, ".$task['square'].' кв.м, в '.$task['num_of_rooms'].'-к квартире';?></a>
                    </h3>
